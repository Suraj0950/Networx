...
let [view, setView] = useState(false)
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true)
    try {
      let loginRes = await axios.post( serverUrl + '/api/auth/login', {
        email,
        password
      }, {
    ...
    <div className="w-full h-screen bg-amber-100 flex flex-col items-center justify-start">
      ...
      <form className='bg-amber-200 w-[90%] max-w-[400px] h-[600px] mt-[20px] md:shadow-xl flex flex-col justify-center gap-[10px] p-[20px] rounded-[20px]'
       onSubmit={handleLogin}>
        <h1 className='text-gray-800 text-[30px] font-semibold mb-[20px]'>SignIn</h1>
        ...
        <button className='w-[100%] h-[50px] rounded-full bg-amber-400 mt-[20px] text-black font-semibold cursor-pointer'> {loading? 'Loading...' : 'SignIn'} </button>
        <p className='text-center mt-[5px] cursor-pointer' onClick={() => navigate("/signup")} >want to create new account? <span className='text-[#1919d2]'> SignUp </span></p>
      </form>
    </div>
  )
}
